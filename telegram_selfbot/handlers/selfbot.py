...
def register_selfbot(client, bot=None):
    @client.on(events.CallbackQuery)
    async def selfbot_callback(event):
        uid = event.sender_id
        ...
        if data == "back_main":
            btns = await panel_mod.get_main_panel_buttons(uid)
        ...
        if data in ["trans_menu", "silence_menu", "anim_menu", "status_menu", "social_menu",
                     "account_menu", "tag_menu", "payment_menu", "timed_msg_menu", "broadcast_menu",
                     "license_menu", "help_menu", "tabchi_menu"]:
            menu_data = {
                "trans_menu": ("🌍 **منوی ترجمه**\nروی یک زبان کلیک کنید.", [[Button.inline("🇬🇧 انگلیسی", "trans_en"), Button.inline("🇸🇦 عربی", "trans_ar")],[Button.inline("🇨🇳 چینی", "trans_zh"), Button.inline("🇷🇺 روسی", "trans_ru")],[Button.inline("🔙 برگشت", "back_main")]]),
                "silence_menu": ("**🤐 منوی سکوت**\n(روی کاربر ریپلای کن و بنویس سکوت)", [[Button.inline("🤐 سکوت (ریپلای)", "silence_perm"), Button.inline("⏱ تایم دار", "silence_timed")],[Button.inline("✅ حذف سکوت", "unsilence"), Button.inline("📋 لیست سکوت", "silence_list_menu")],[Button.inline("🔙 برگشت", "back_main")]]),
                "status_menu": ("**🎮 منوی وضعیت**\n(اسم پروفایل با وضعیت آپدیت میشه)", [[Button.inline("🎮 درحال بازی", "set_status_playing"), Button.inline("⌨️ درحال تایپ", "set_status_typing")],[Button.inline("📺 درحال ویدیو", "set_status_watching"), Button.inline("🎵 گوش دادن اهنگ", "set_status_music")],[Button.inline("🔙 برگشت", "back_main")]]),
                "anim_menu": ("**✨ منوی انیمیشن**", [[Button.inline("❤️ قلب", "anim_heart"), Button.inline("🌊 موج", "anim_wave")],[Button.inline("💓 ضربان", "anim_beat"), Button.inline("💝 Love", "anim_love")],[Button.inline("🌙 ماه", "anim_moon"), Button.inline("🌕 Moon2", "anim_moon2")],[Button.inline("🕐 Clock2", "anim_clock2"), Button.inline("⚡ رعد", "anim_thunder")],[Button.inline("🔙 برگشت", "back_main")]]),
                "help_menu": ("**📚 راهنما**\nروی هر گزینه کلیک کنید.", [[Button.inline("🌍 ترجمه", "help_translate"), Button.inline("🔒 قفل پیوی", "help_pm_lock")],[Button.inline("🤐 سکوت", "help_silence"), Button.inline("👤 کپی", "help_copy")],[Button.inline("📢 تگ", "help_tag"), Button.inline("💳 پرداخت", "help_payment")],[Button.inline("🔙 برگشت", "back_main")]]),
                "tag_menu": ("**📢 منوی تگ**\n(فقط در گروه کار میکنه)", [[Button.inline("📢 تگ همه", "tag_all"), Button.inline("👤 تگ ادمین ها", "tag_admins")],[Button.inline("🔙 برگشت", "back_main")]]),
            }
            if data in menu_data:
                txt, btns = menu_data[data]
        ...
        if data == "pm_lock_toggle":
            ...
            await event.answer(f"قفل پیوی: {'✅ روشن' if new == 'on' else '❌ خاموش'}", alert=True)
            return
        ...
        if data == "toggle_selfbot":
            ...
            await event.answer(f"سلف‌بات: {'✅ فعال' if new == 'on' else '❌ غیرفعال'}", alert=True)
            return
        ...
        if data == "toggle_dot":
            ...
            await event.answer(f"دات: {'✅ روشن' if new == 'on' else '❌ خاموش'}", alert=True)
            return
        ...
        if data == "toggle_clock_bio":
            ...
            if new == "on":
                try:
                    ...
                    set_setting(uid, "base_first_name", me.first_name or "")
                except:
                    pass
            await event.answer(f"ساعت کنار اسم: {'✅ روشن' if new == 'on' else '❌ خاموش'}", alert=True)
            return
        ...
        if data == "toggle_thunder_bio":
            ...
            if new == "on":
                try:
                    me = await client.get_me()
                    set_setting(uid, "base_bio", me.about or "")
                except:
                    pass
            await event.answer(f"رعد در بیو: {'✅ روشن' if new == 'on' else '❌ خاموش'}", alert=True)
            return

        if data == "toggle_auto_translate":
            current = get_setting(uid, "auto_translate", "off")
            new = "off" if current == "on" else "on"
            set_setting(uid, "auto_translate", new)
            if new == "on":
                btns = [
                    [Button.inline("🇬🇧 انگلیسی", "set_trans_lang_en"), Button.inline("🇸🇦 عربی", "set_trans_lang_ar")],
                    [Button.inline("🇨🇳 چینی", "set_trans_lang_zh"), Button.inline("🇷🇺 روسی", "set_trans_lang_ru")],
                    [Button.inline("🔙 برگشت", "back_main")]
                ]
                await event.edit("🌍 **ترجمه خودکار روشن شد!**\nزبان مقصد را انتخاب کنید:", buttons=btns)
            else:
                await event.answer(f"ترجمه خودکار: ❌ خاموش", alert=True)
            return

        if data.startswith("set_trans_lang_"):
            lang = data.split("_")[3]
            names = {"en": "🇬🇧 انگلیسی", "ar": "🇸🇦 عربی", "zh": "🇨🇳 چینی", "ru": "🇷🇺 روسی"}
            set_setting(uid, "auto_translate_lang", lang)
            await event.answer(f"✅ زبان ترجمه: {names.get(lang, lang)}", alert=True)
            btns = await panel_mod.get_main_panel_buttons(uid)
            await event.edit(f"✅ **زبان ترجمه به {names.get(lang, lang)} تنظیم شد!**\nحالا هر متنی بفرستی ترجمه میشه.", buttons=btns)
            return

        if data == "toggle_auto_save":
            current = get_setting(uid, "auto_save_timed", "off")
            new = "off" if current == "on" else "on"
            set_setting(uid, "auto_save_timed", new)
            await event.answer(f"سیو تایم دار: {'✅ روشن' if new == 'on' else '❌ خاموش'}", alert=True)
            return
        ...
        if data == "silence_list_menu":
            ...
            msg = "**📋 لیست سکوت:**\n\n"
            for r in rows:
                until = r[1] if r[1] != "forever" else "دائمی"
                msg += f"👤 {r[0]} - تا {until}\n"
        ...
        help_texts = {
            "help_translate": "🌍 **ترجمه**\nدستور: `.en متن` برای انگلیسی\n`.ar متن` برای عربی\nاز پنل هم میتونی انتخاب کنی",
            "help_pm_lock": "🔒 **قفل پیوی**\nاز پنل فعال/غیرفعال کن یا بنویس `.pmlock`",
            "help_silence": "🤐 **سکوت**\nروی کاربر ریپلای کن و بنویس `سکوت`\n`سکوت 5` برای تایم دار\n`حذف سکوت` برای برداشتن",
            "help_copy": "👤 **کپی پروفایل**\nاز پنل روشن کن، روی کاربر ریپلای کن تا کپی بشه",
            "help_tag": "📢 **تگ**\n`تگ همه` برای همه اعضا\n`تگ ادمین` برای ادمین‌ها",
            "help_payment": "💳 **پرداخت**\nاز ربات تلگرامی استفاده کن",
        }
        if data in help_texts:
            await event.edit(help_texts[data], buttons=[[Button.inline("🔙 برگشت راهنما", "help_menu")]])
            return

        if data == "silence_perm":
            await event.answer("⚠️ روی کاربر ریپلای کن و بنویس «سکوت»", alert=True)
            return
        if data == "silence_timed":
            await event.answer("⚠️ روی کاربر ریپلای کن و بنویس «سکوت 5»", alert=True)
            return
        if data == "unsilence":
            await event.answer("⚠️ روی کاربر ریپلای کن و بنویس «حذف سکوت»", alert=True)
            return
        if data in ["tag_all", "tag_admins"]:
            await event.answer("⚠️ فقط در گروه کار میکنه", alert=True)
            return

        if data in ["set_status_playing", "set_status_typing", "set_status_watching", "set_status_music"]:
            stypes = {"set_status_playing": "playing", "set_status_typing": "typing", "set_status_watching": "watching", "set_status_music": "music"}
            stexts = {"set_status_playing": "🎮 درحال بازی", "set_status_typing": "⌨️ درحال تایپ", "set_status_watching": "📺 درحال ویدیو دیدن", "set_status_music": "🎵 درحال گوش دادن به اهنگ"}
        ...
        await event.answer("⚠️ این بخش در ربات تلگرامی کاملتر است.", alert=True)
    ...
    async def handle_text_input(event):
        ...
        cmds_map = {
            "panel": cmd_panel, "start": cmd_panel, "پنل": cmd_panel,
            "pmlock": cmd_toggle_pmlock, "lock": cmd_toggle_pmlock, "nopm": cmd_toggle_pmlock, "قفل": cmd_toggle_pmlock,
            "silence": cmd_silence, "unsilence": cmd_unsilence, "سکوت": cmd_silence, "حذف سکوت": cmd_unsilence,
            "copy": cmd_copy_on, "copyoff": cmd_copy_off, "کپی": cmd_copy_on, "کپی خاموش": cmd_copy_off,
            "tagall": cmd_tagall, "tagadmins": cmd_tag_admins, "تگ همه": cmd_tagall, "تگ ادمین": cmd_tag_admins,
            "download": cmd_download, "save": cmd_save, "دانلود": cmd_download, "ذخیره": cmd_save,
            "repeat": cmd_repeat, "savetimed": cmd_save_timed, "تکرار": cmd_repeat, "سیو تایم": cmd_save_timed,
            ...
            "بازی": cmd_set_status, "ویدیو": cmd_set_status, "اهنگ": cmd_set_status,
            "setname": cmd_set_name, "setlastname": cmd_set_lastname,
            "نام": cmd_set_name, "نام خانوادگی": cmd_set_lastname,
            "setphoto": cmd_set_photo, "delphoto": cmd_del_photo, "delallphotos": cmd_del_all_photos,
            "عکس": cmd_set_photo, "حذف عکس": cmd_del_photo, "حذف همه عکس": cmd_del_all_photos,
            "friend": cmd_add_friend_text, "enemy": cmd_add_enemy_text, "crush": cmd_add_crush_text,
            "دوست": cmd_add_friend_text, "دشمن": cmd_add_enemy_text, "کراش": cmd_add_crush_text,
            "help": cmd_help, "admin": cmd_admin_panel,
            "راهنما": cmd_help, "ادمین": cmd_admin_panel,
            "creategroup": cmd_create_group, "createchannel": cmd_create_channel,
            "deletechat": cmd_delete_chat, "ساخت گروه": cmd_create_group, "ساخت کانال": cmd_create_channel,
        }
    ...
    async def handle_reply_actions(event):
        ...
        if event.is_reply:
            ...
            if text in ["download", ".download", "دانلود"]:
                if reply.media:
            ...
            if text.startswith("silence") or text.startswith(".silence") or text.startswith("سکوت"):
                parts = text.split()
    ...
    async def status_updater(event):
        ...
        if stype != "none":
            try:
                await client(telethon.tl.functions.account.UpdateStatusRequest(offline=False))
                st = get_setting(uid, "custom_status_text", "")
                emoji = "🎮" if stype == "playing" else "⌨️" if stype == "typing" else "📺" if stype == "watching" else "🎵" if stype == "music" else "📸" if stype == "photo" else "😎"
                base = get_setting(uid, "base_first_name", "")
                if base and st:
                    new = f"{base} | {emoji} {st}"
                    me = await client.get_me()
                    if me.first_name != new:
                        await client.edit_profile(first_name=new)
            except:
                pass
    ...
    async def auto_translate_handler(event):
        uid = event.sender_id
        if get_setting(uid, "auto_translate", "off") != "on":
            return
        text = event.message.text.strip()
        if not text or text.lower() in ["پنل", "panel", "راهنما", "help", "/start"]:
            return
        dot = get_setting(uid, "dot_mode", "on")
        if dot == "on" and text.startswith("."):
            return
        target = get_setting(uid, "auto_translate_lang", "en")
        translated = translate_text(text, target)
        if translated and translated != text:
            await event.edit(translated)

    @client.on(events.NewMessage())
    async def timed_msg_saver(event):
        uid = event.sender_id
        if get_setting(uid, "auto_save_timed", "off") != "on":
            return
        ttl = getattr(event.message, 'ttl_seconds', 0)
        if ttl and ttl > 0:
            try:
    ...
    async def clock_bio_task():
        ...
        while True:
            try:
                if get_setting(uid, "clock_in_bio", "off") == "on":
                    me = await client.get_me()
                    clock = format_time(1)
                    base = get_setting(uid, "base_first_name", "")
                    if not base:
                        base = me.first_name or ""
                        set_setting(uid, "base_first_name", base)
                    new_name = f"{base} 🕐 {clock}"
    ...
    async def thunder_bio_task():
        ...
        while True:
            try:
                if get_setting(uid, "thunder_in_bio", "off") == "on":
                    me_full = await client.get_me()
                    base = get_setting(uid, "base_bio", "")
                    if not base:
                        base = me_full.about or ""
                        set_setting(uid, "base_bio", base)
                    new_bio = f"{base} ⚡" if state else base
                    if (me_full.about or "") != new_bio:
                        try:
    ...
    asyncio.ensure_future(clock_bio_task())
    asyncio.ensure_future(thunder_bio_task())
