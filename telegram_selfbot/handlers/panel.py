...
async def get_main_panel_buttons(user_id):
    ...
    auto_tr = get_setting(user_id, "auto_translate", "off")
    tr_label = "✅ روشن" if auto_tr == "on" else "❌ خاموش"
    auto_save = get_setting(user_id, "auto_save_timed", "off")
    save_label = "✅ روشن" if auto_save == "on" else "❌ خاموش"
    ...
    buttons = [
        ...
        [Button.inline("📤 تپچی", "tabchi_menu"), Button.inline("📶 پینگ", "ping_panel")],
        [Button.inline(f"🕐 ساعت کنار اسم {clock_label}", "toggle_clock_bio"), Button.inline(f"⚡ رعد در بیو {thunder_label}", "toggle_thunder_bio")],
        [Button.inline(f"🌍 ترجمه خودکار {tr_label}", "toggle_auto_translate"), Button.inline(f"⏰ سیو تایم دار {save_label}", "toggle_auto_save")],
    ...
    buttons.append([Button.inline("❌ بستن", "close_panel")])
    return buttons
