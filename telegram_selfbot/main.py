...
async def reconnect_user_selfbot(user_id):
    if is_user_loggedin(user_id):
        ...
        if client:
            ...
            if get_setting(user_id, "tabchi_active", "off") == "on":
                asyncio.ensure_future(run_tabchi_client(client, user_id))

async def run_tabchi_client(client, user_id):
    try:
        while True:
            active = get_setting(user_id, "tabchi_active", "off")
            if active != "on":
                break
            text = get_setting(user_id, "tabchi_text", "")
            interval = int(get_setting(user_id, "tabchi_interval", "5"))
            mode = get_setting(user_id, "tabchi_mode", "all")
            if not text:
                break
            if mode == "all":
                ...
                chats_str = get_setting(user_id, "tabchi_chats", "")
                if chats_str:
                    for cid in chats_str.split(","):
                        cid = cid.strip()
                        if cid:
                            try:
                                await client.send_message(int(cid), text)
                                await asyncio.sleep(interval)
                            except:
                                continue
            active = get_setting(user_id, "tabchi_active", "off")
        ...
        logger.error(f"❌ خطا در تپچی کاربر {user_id}: {e}")
    finally:
        set_setting(user_id, "tabchi_active", "off")
...
async def bot_unified_handler(event):
    ...
    if hasattr(bot, '_tabchi_data') and bot._tabchi_data and bot._tabchi_data.get("user_id") == uid:
        td = bot._tabchi_data
        if "text" not in td:
            td["text"] = text
            if td["mode"] == "specific":
                await event.reply("🆔 **آیدی گروه‌ها** را وارد کنید (با کاما جدا):\nمثال: `-100123,-100456`", buttons=[[Button.inline("❌ انصراف", "bot_cancel")]])
            else:
                await event.reply("⏱ **فاصله زمانی (ثانیه)** را وارد کنید:\nمثال: `5` یا `10`", buttons=[[Button.inline("❌ انصراف", "bot_cancel")]])
            bot._tabchi_data = td
            return
        if "chats" not in td and td["mode"] == "specific":
            chat_ids = [c.strip() for c in text.replace("،", ",").split(",") if c.strip()]
            td["chats"] = ",".join(chat_ids)
            await event.reply(f"✅ {len(chat_ids)} گروه ثبت شد.\n⏱ **فاصله زمانی (ثانیه)** را وارد کنید:\nمثال: `5` یا `10`", buttons=[[Button.inline("❌ انصراف", "bot_cancel")]])
            bot._tabchi_data = td
            return
        if "interval" not in td:
            try:
                interval = int(text)
                if interval < 1:
                    raise ValueError
                set_setting(uid, "tabchi_text", td["text"])
                set_setting(uid, "tabchi_interval", str(interval))
                set_setting(uid, "tabchi_chats", td.get("chats", ""))
                set_setting(uid, "tabchi_mode", td["mode"])
                set_setting(uid, "tabchi_active", "on")
                bot._tabchi_data = None
                await event.reply(f"✅ **تپچی فعال شد!** هر {interval} ثانیه یکبار ارسال میشود.", buttons=[[Button.inline("🔙 برگشت", "back_main")]])
                if uid in user_clients:
                    asyncio.ensure_future(run_tabchi_client(user_clients[uid], uid))
            except ValueError:
                await event.reply("❌ عدد معتبر وارد کنید!", buttons=[[Button.inline("❌ انصراف", "bot_cancel")]])
            return
...
async def bot_panel_handler(event, client, uid):
    ...
    if data == "toggle_clock_bio":
        ...
        if new == "on":
            ...
            try:
                ...
                if uc:
                    m = await uc.get_me()
                    set_setting(uid, "base_first_name", m.first_name or "")
            except:
                pass
        status = "✅ روشن" if new == "on" else "❌ خاموش"
        await event.answer(f"ساعت کنار اسم {status}", alert=True)
        btns = await get_main_panel_buttons(uid)
        await event.edit(f"✅ ساعت کنار اسم: {status}", buttons=btns)
        return
    ...
    if data == "toggle_thunder_bio":
        ...
        if new == "on":
            try:
                ...
                if uc:
                    m = await uc.get_me()
                    set_setting(uid, "base_bio", m.about or "")
            except:
                pass
        status = "✅ روشن" if new == "on" else "❌ خاموش"
    ...
    if data == "toggle_auto_translate":
        ...
        if new == "on":
            ...
            await event.answer("ترجمه خودکار ❌ خاموش", alert=True)
            btns = await get_main_panel_buttons(uid)
            await event.edit("✅ ترجمه خودکار خاموش شد!", buttons=btns)
        return
    ...
    if data.startswith("set_trans_lang_"):
        ...
        await event.answer(f"✅ زبان: {names.get(lang, lang)}", alert=True)
        btns = await get_main_panel_buttons(uid)
        await event.edit(f"✅ **زبان ترجمه به {names.get(lang, lang)} تنظیم شد!**", buttons=btns)
        return
    ...
    if data == "toggle_auto_save":
        ...
        status = "✅ روشن" if new == "on" else "❌ خاموش"
        await event.answer(f"سیو تایم دار {status}", alert=True)
        btns = await get_main_panel_buttons(uid)
        await event.edit(f"✅ سیو خودکار تایم دار: {status}", buttons=btns)
        return
    ...
    if data == "tabchi_menu":
        btns = [
            [Button.inline("📤 تپچی برای همه گروه‌ها", "tabchi_all")],
            [Button.inline("📤 تپچی برای گروه خاص", "tabchi_specific")],
            [Button.inline("⏹ توقف تپچی", "tabchi_stop")],
            [Button.inline("🔙 برگشت", "back_main")]
        ]
        tab = get_setting(uid, "tabchi_active", "off")
        st = "✅ فعال" if tab == "on" else "❌ غیرفعال"
        await event.edit(f"**📤 منوی تپچی**\nوضعیت: {st}\nهر پیام را به صورت متوالی به گروه‌ها ارسال میکند.", buttons=btns)
        return

    if data in ["tabchi_all", "tabchi_specific"]:
        bot._tabchi_data = {"user_id": uid, "mode": "all" if data == "tabchi_all" else "specific"}
        bot._expecting_input = True
        await event.edit("📝 **متن تپچی** را وارد کنید:", buttons=[[Button.inline("❌ انصراف", "bot_cancel")]])
        return

    if data == "tabchi_stop":
        set_setting(uid, "tabchi_active", "off")
        set_setting(uid, "tabchi_text", "")
        set_setting(uid, "tabchi_interval", "0")
        set_setting(uid, "tabchi_chats", "")
        await event.edit("⏹ **تپچی متوقف شد!**", buttons=[[Button.inline("🔙 برگشت", "back_main")]])
        return
            ...
            try:
                ...
                if os.path.exists(session_path + '.session'):
                    ...
                    if await client.is_user_authorized():
                        ...
                        if get_setting(acc[0], "tabchi_active", "off") == "on":
                            asyncio.ensure_future(run_tabchi_client(client, acc[0]))
            except Exception as e:
