...
from handlers.panel import register_handlers
...
async def status_updater(event):
    ...
    if stype != "none":
        stext = get_setting(uid, "custom_status_text", "")
        try:
            if stype == "playing":
                await client(telethon.tl.functions.account.UpdateStatusRequest(offline=False))
            elif stype == "typing":
...
async def fun_extra_handler(event):
    uid = event.sender_id
    text = event.message.text.strip()
    if text == "love" or text == ".love":
        await animate_love(event)
        return
    if text == "moon" or text == ".moon":
        await animate_moon(event)
        return
    if text == "moon2" or text == ".moon2":
        await animate_moon2(event)
        return
    if text == "clock2" or text == ".clock2":
        await animate_clock2(event)
        return
    if text == "thunder" or text == ".thunder":
        await animate_thunder(event)
        return
    if text == "earth" or text == ".earth":
        await animate_earth(event)
        return
...
async def main():
    ...
    me = await client.get_me()
    client._self_id = me.id
    print(f"✅ ربات با موفقیت وارد شد! {me.first_name} (@{me.username})")

    register_handlers(client)

    if LOG_GROUP_ID:
        try:
            await client.send_message(LOG_GROUP_ID, f"✅ **ربات روشن شد!**\nزمان: {format_time(3)}")
        except:
            pass
