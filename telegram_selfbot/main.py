...
from config import API_ID, API_HASH, PHONE_NUMBER, SESSION_NAME, ADMINS, OWNER_ID, BOT_TOKEN, LOG_GROUP_ID, FORCE_JOIN_CHATS
...
async def start_user_selfbot(user_id, phone):
    session_path = os.path.join('sessions', f'user_{user_id}')
    client = TelegramClient(session_path, API_ID, API_HASH)
    ...
    register_selfbot(client)
    user_clients[user_id] = client
    add_user_account(user_id, phone, session_path, OWNER_ID)
    logger.info(f"✅ سلف‌بات کاربر {user_id} ({me.first_name}) راه‌اندازی شد!")
    return client
...
async def bot_unified_handler(event):
    ...
    if step == "waiting_code":
        ...
        temp_session = login_data.get("temp_session", "")
        try:
            ...
            await start_user_selfbot(uid, phone)
            clear_login_state(uid)
            btns = await get_main_panel_buttons(uid)
...
async def main():
    ...
    if PHONE_NUMBER:
        owner_client = TelegramClient(SESSION_NAME, API_ID, API_HASH)
        await owner_client.start(phone=PHONE_NUMBER)
        me = await owner_client.get_me()
        register_selfbot(owner_client)
        user_clients[OWNER_ID] = owner_client
        print(f"✅ سلف‌بات ادمین روشن شد: {me.first_name} (@{me.username})")
